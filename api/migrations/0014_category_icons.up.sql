-- Replace emoji glyphs with Iconify icon names (Font Awesome Free Solid).
-- The frontend resolves these as `fa7-solid:<name>` via astro-icon, bundling
-- only referenced icons at build time (no external CDN, no runtime fetch).
